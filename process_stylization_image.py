...
def stylization(stylization_module, smoothing_module, content_image_path, style_image_path, content_seg_path, style_seg_path,
                cuda, cont_seg_remapping=None, styl_seg_remapping=None):
    # Load image
    with torch.no_grad():
        try:
            cont_img = Image.open(content_image_path).convert('RGB')
        except:
            cont_img = content_image_path
        try:
            styl_img = Image.open(style_image_path).convert('RGB')
        except:
            styl_img = style_image_path
        ...
        try:
            cont_seg = content_seg_path
            styl_seg = style_seg_path
